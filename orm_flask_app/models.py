...
class SampleSong(db.Model):
    __tablename__ = 'sampleSong'
    source_id = db.Column(db.Integer, db.ForeignKey('sampleSource.source_id'))
    song_id = db.Column(db.Integer, db.ForeignKey('song.song_id'), primary_key=True)
    ISRC = db.Column(db.String, db.ForeignKey('song.ISRC'))

class MySong(db.Model):
    __tablename__ = 'mySong'
    song_id = db.Column(db.Integer, db.ForeignKey('song.song_id'), primary_key=True)
    # change to Boolean needed?
    finished = db.Column(db.Integer)  # 0 = no, 1 = yes
